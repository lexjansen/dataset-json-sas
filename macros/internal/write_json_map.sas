              ...
              %do %while(&vrc eq 0);
                ...
                write open object;
                  ...
                  %if &INFORMAT ne %then %do;
                    write values "INFORMAT";
                    write open array;
                      %let _informat=%scan(&INFORMAT, 1, %str( ));
                      %let _length=%scan(&INFORMAT, 2, %str( ));
                      %let _width=%scan(&INFORMAT, 3, %str( ));
                      write values &_informat &_length &_width;
                    write close;
                  %end;    
                write close;
                %let vrc=%sysfunc(fetch(&vdsid));
